...
func (tq *TransactionsQuery) QueryUsers() *UsersQuery {
	query := (&UsersClient{config: tq.config}).Query()
...
func (tq *TransactionsQuery) WithUsers(opts ...func(*UsersQuery)) *TransactionsQuery {
	query := (&UsersClient{config: tq.config}).Query()
...
//		GroupBy(transactions.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transactions.Query().
//		Select(transactions.FieldUserID).
//		Scan(ctx, &v)
...
func (tq *TransactionsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transactions, error) {
	...
	if query := tq.withUsers; query != nil {
		if err := tq.loadUsers(ctx, query, nodes, nil,
			func(n *Transactions, e *Users) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionsQuery) loadUsers(ctx context.Context, query *UsersQuery, nodes []*Transactions, init func(*Transactions), assign func(*Transactions, *Users)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transactions)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (tq *TransactionsQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withUsers != nil {
			_spec.Node.AddColumnOnce(transactions.FieldUserID)
		}
	}
