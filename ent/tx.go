...
func (tx *Tx) init() {
	tx.AccountBalances = NewAccountBalancesClient(tx.config)
	tx.AccountDetails = NewAccountDetailsClient(tx.config)
	tx.AccountFlags = NewAccountFlagsClient(tx.config)
	tx.Accounts = NewAccountsClient(tx.config)
	tx.Banners = NewBannersClient(tx.config)
	tx.DebitCardDesign = NewDebitCardDesignClient(tx.config)
	tx.DebitCardDetails = NewDebitCardDetailsClient(tx.config)
	tx.DebitCardStatus = NewDebitCardStatusClient(tx.config)
	tx.DebitCards = NewDebitCardsClient(tx.config)
	tx.Transactions = NewTransactionsClient(tx.config)
	tx.UserGreetings = NewUserGreetingsClient(tx.config)
...
// applies a query, for example: AccountBalances.QueryXXX(), the query will be executed
